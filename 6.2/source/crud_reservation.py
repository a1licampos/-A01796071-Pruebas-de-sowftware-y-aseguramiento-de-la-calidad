...
class SystemReservation:
    ...
    def _csv_to_pandas(self) -> Tuple[pd.DataFrame,
                                      pd.DataFrame,
                                      pd.DataFrame]:
        """Return pandas from csv file."""
        df1 = pd.read_csv(f"{self.folder_tests}/reservations.csv",
                          encoding="utf-8")
        df2 = pd.read_csv(f"{self.folder_tests}/hotel.csv",
                          encoding="utf-8")
        df3 = pd.read_csv(f"{self.folder_tests}/customers.csv",
                          encoding="utf-8")
    ...
    def _pandas_to_csv(self, df: pd.DataFrame) -> None:
        ...
        try:
            df.to_csv(f"{self.folder_tests}/reservations.csv",
                      index=False,
                      encoding="utf-8")
    ...
    def create_reservation(self,
        ...
        if c_phone in df_reservations["Cliente_telefono"].values:
            ...
            if row_reservations.iloc[0]["Estatus"]:
                print(
                    ...
                    f"ya tiene una reservación en '{h_name}' activa"
                )
                return False
        ...
        df = pd.concat(
            [df_reservations, pd.DataFrame([new_data])],
            ignore_index=True)

        print(
            f"\tRevervacion exitosa: "
            f"hotel: {h_name} & cliente tel:{c_phone}"
        )
