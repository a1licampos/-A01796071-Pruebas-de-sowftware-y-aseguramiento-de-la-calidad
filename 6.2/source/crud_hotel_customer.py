...
class CRUDSystemHotelCustomer:
    ...
    def _csv_to_pandas(self) -> pd.DataFrame:
        ...
        if self.hotel == 1:
            return pd.read_csv(f"{self.folder_tests}/hotel.csv",
                               encoding="utf-8")

        return pd.read_csv(f"{self.folder_tests}/customers.csv",
                           encoding="utf-8")
    ...
    def _pandas_to_csv(self, df: pd.DataFrame) -> None:
        ...
        if self.hotel == 1:
            try:
                df.to_csv(f"{self.folder_tests}/hotel.csv",
                          index=False,
                          encoding="utf-8")
            ...
            try:
                df.to_csv(f"{self.folder_tests}/customers.csv",
                          index=False,
                          encoding="utf-8")
    ...
    def check_no_duplicates(self,
        ...
        """Check there is not duplicates registers."""
    ...
    def get_register(self,
                     h_name: Optional[str] = None,
                     c_phone: Optional[int] = None) -> pd.DataFrame:
    ...
    def set_register(self,
        ...
        if self.hotel == 1:
            if (
                h_name is None
                or h_no_stars is None
                or h_price_per_night is None
            ):
                print(
                    "\tFaltan argumentos hotel: "
                    "name, no_stars y price_per_night."
                )
                self._print_line_break()
                return
            ...
            if c_name is None or c_last_name is None or c_phone is None:
                print(
                    "\tFaltan argumentos cliente: "
                    "name, last_name y phone."
                )
                self._print_line_break()
                return
        ...
        if self.hotel == 1:
            if self.check_no_duplicates(h_name=h_name):
                ...
                if h_no_stars is not None and h_no_stars > 0:
                    ...
                    print(
                        "\tNo estrellas no es "
                        "un dato válido o no fue proporcionado"
                    )
                ...
                if h_price_per_night is not None and h_price_per_night > 0:
                    ...
                    print(
                        f"\t[Hotel] Precio por "
                        f"noche: {h_price_per_night} actualizado"
                    )
                else:
                    print(
                        "\tPrecio por noche no "
                        "es un dato válido o no fue proporcionado"
                    )
            ...
            if self.check_no_duplicates(c_phone=c_phone):
                ...
                if c_name is not None:
                    ...
                    print(
                        "\tEl nombre no es un dato"
                        "válido o no fue proporcionado"
                    )
                ...
                if c_last_name is not None:
                    ...
                    print(
                        "\tEl apellido no es un dato"
                        "válido o no fue proporcionado"
                    )

                if (
                    c_new_phone is not None
                    and int(math.log10(c_new_phone)) + 1 == 10
                ):
                    df.loc[df["Telefono"] == c_phone, "Telefono"] = c_new_phone
                    print(f"\t[Client] Telefono: {c_new_phone} actualizado")
                else:
                    print(
                        "\tEl teléfono del cliente no"
                        "es un dato válido o no fue proporcionado"
                    )
